const projects = [
  {
    ...
    description: {
      ko: `90*50mm 또는 85*55mm는 물론 규격의 근사치 조차 따르지 않은, 명함이라 불리지만 명함의 핵심 기능을 전혀 하지 못하는, 네 개의 모서리를 가진 작은 널빤지에 불과한 이것을 내밀었을 때 받는 이들의 반응을 상상한다. 웃거나 당황하거나 가지고 싶다거나 화를 내거나. 
      
이걸 명함이라고 만드신 건가요? 전화번호는요? 메일 주소는 지예님 게 아니던데요? 제가 가지고 갈 수도 없잖아요. 곤란하게 됐습니다. 제 명함은 점점 자라나는데 지금보다 작아질 수 없다는 사실은 확실해요. 대신 다가오는 여름을 맞아 땀을 식히는 부채로 쓴다거나 햇빛 가리개로 쓴다거나 가끔 길바닥에 앉고 싶을 때 간이 방석으로 사용할 수 있습니다. 꽤 유용하게 쓰일지도 몰라요.`,
      en: "."
    },
    images: [
...
`
  ...
  },*/
  {
    ...
    description: {
      ko: "최초의 오선 위에 꺾인 오선 위에 뒤집힌 오선 위에 잘린 오선 위에 바랜 오선 위에 번진 오선 위에 조각난 오선 위에 코드 사라진 오선 위에 기능 잃은 오선 위에 마침 없는 오선 위의 마침 없는 무한한 되돌이표 ",
      en: "Not yet."
    },
    images: [
...
— 한지은(2026), 「한국 진 문화의 지형도 그리기」, 석사 학위 논문, 홍익대학교,44p

그럼에도 사라지기는 커녕 지속되고 있는 이유는 진이 하나로 정의 내릴 수 없는 혼돈이자 좋아하는 것을 동료들과 나누기 위해 기꺼이 만드는 실천이라는 불변의 태도이기 때문일 것이다. 이러한 태도 앞에서 더 이상의 화이트 큐브는 필요하지 않다.

[⌜서신교환⌟](https://www.instagram.com/exchange.papers/)의 첫 활동으로 전자 음악가 레이 하라카미의 음악에 대한 나의 생각과 사랑을 담은 팬진을 만들고자 했다. 의지가 무색하게 구글의 알고리즘이 선별한 콘텐츠와 이미지로 내용을 구성한 점은 여전히 아쉽지만 무언가를 만들고자 하는 마음 하나면 완성도와 퀄리티는 중요하지 않은 것이 진을 만드는 정신이거늘..
